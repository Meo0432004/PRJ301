...
public class HomeController extends HttpServlet {
    ...
    void productList(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("admin")==null && session.getAttribute("user")==null && session.getAttribute("guest")==null){
            session.setAttribute("guest", "guest");
            Cookie cookies[] = request.getCookies();
            for (Cookie c : cookies) {
                if(c.getName().equals("admin")){
                    session.setAttribute("admin", c.getValue());
        ...
        List<Product> listP = pdao.getAllProduct(type);
        List<Category> listC = cdao.getAllCategory();
        Product newP = pdao.getNewestProduct();
